// Copyright (c), CommunityLogiq Software
//
// THIS FILE IS AUTOGENERATED, DO NOT EDIT

// automatically generated by the FlatBuffers compiler, do not modify

/* eslint-disable @typescript-eslint/no-unused-vars, @typescript-eslint/no-explicit-any, @typescript-eslint/no-non-null-assertion */

import * as flatbuffers from 'flatbuffers/js/flatbuffers';



export class VFixedSizeBytes implements flatbuffers.IUnpackableObject<VFixedSizeBytesT> {
  bb: flatbuffers.ByteBuffer|null = null;
  bb_pos = 0;
  __init(i:number, bb:flatbuffers.ByteBuffer):VFixedSizeBytes {
  this.bb_pos = i;
  this.bb = bb;
  return this;
}

static getRootAsVFixedSizeBytes(bb:flatbuffers.ByteBuffer, obj?:VFixedSizeBytes):VFixedSizeBytes {
  return (obj || new VFixedSizeBytes()).__init(bb.readInt32(bb.position()) + bb.position(), bb);
}

static getSizePrefixedRootAsVFixedSizeBytes(bb:flatbuffers.ByteBuffer, obj?:VFixedSizeBytes):VFixedSizeBytes {
  bb.setPosition(bb.position() + flatbuffers.SIZE_PREFIX_LENGTH);
  return (obj || new VFixedSizeBytes()).__init(bb.readInt32(bb.position()) + bb.position(), bb);
}

v(index: number):number|null {
  const offset = this.bb!.__offset(this.bb_pos, 4);
  return offset ? this.bb!.readUint8(this.bb!.__vector(this.bb_pos + offset) + index) : 0;
}

vLength():number {
  const offset = this.bb!.__offset(this.bb_pos, 4);
  return offset ? this.bb!.__vector_len(this.bb_pos + offset) : 0;
}

vArray():Uint8Array|null {
  const offset = this.bb!.__offset(this.bb_pos, 4);
  return offset ? new Uint8Array(this.bb!.bytes().buffer, this.bb!.bytes().byteOffset + this.bb!.__vector(this.bb_pos + offset), this.bb!.__vector_len(this.bb_pos + offset)) : null;
}

sz():number {
  const offset = this.bb!.__offset(this.bb_pos, 6);
  return offset ? this.bb!.readInt32(this.bb_pos + offset) : 0;
}

static startVFixedSizeBytes(builder:flatbuffers.Builder) {
  builder.startObject(2);
}

static addV(builder:flatbuffers.Builder, vOffset:flatbuffers.Offset) {
  builder.addFieldOffset(0, vOffset, 0);
}

static createVVector(builder:flatbuffers.Builder, data:number[]|Uint8Array):flatbuffers.Offset {
  builder.startVector(1, data.length, 1);
  for (let i = data.length - 1; i >= 0; i--) {
    builder.addInt8(data[i]!);
  }
  return builder.endVector();
}

static startVVector(builder:flatbuffers.Builder, numElems:number) {
  builder.startVector(1, numElems, 1);
}

static addSz(builder:flatbuffers.Builder, sz:number) {
  builder.addFieldInt32(1, sz, 0);
}

static endVFixedSizeBytes(builder:flatbuffers.Builder):flatbuffers.Offset {
  const offset = builder.endObject();
  builder.requiredField(offset, 4) // v
  return offset;
}

static createVFixedSizeBytes(builder:flatbuffers.Builder, vOffset:flatbuffers.Offset, sz:number):flatbuffers.Offset {
  VFixedSizeBytes.startVFixedSizeBytes(builder);
  VFixedSizeBytes.addV(builder, vOffset);
  VFixedSizeBytes.addSz(builder, sz);
  return VFixedSizeBytes.endVFixedSizeBytes(builder);
}

unpack(): VFixedSizeBytesT {
  return new VFixedSizeBytesT(
    this.bb!.createScalarList<number>(this.v.bind(this), this.vLength()),
    this.sz()
  );
}


unpackTo(_o: VFixedSizeBytesT): void {
  _o.v = this.bb!.createScalarList<number>(this.v.bind(this), this.vLength());
  _o.sz = this.sz();
}
}

export class VFixedSizeBytesT implements flatbuffers.IGeneratedObject {
constructor(
  public v: (number)[] = [],
  public sz: number = 0
){}


pack(builder:flatbuffers.Builder): flatbuffers.Offset {
  const v = VFixedSizeBytes.createVVector(builder, this.v);

  return VFixedSizeBytes.createVFixedSizeBytes(builder,
    v,
    this.sz
  );
}
}