// Copyright (c), CommunityLogiq Software
//
// THIS FILE IS AUTOGENERATED, DO NOT EDIT

// automatically generated by the FlatBuffers compiler, do not modify

/* eslint-disable @typescript-eslint/no-unused-vars, @typescript-eslint/no-explicit-any, @typescript-eslint/no-non-null-assertion */

import * as flatbuffers from 'flatbuffers/js/flatbuffers';



export class EmbeddedTable implements flatbuffers.IUnpackableObject<EmbeddedTableT> {
  bb: flatbuffers.ByteBuffer|null = null;
  bb_pos = 0;
  __init(i:number, bb:flatbuffers.ByteBuffer):EmbeddedTable {
  this.bb_pos = i;
  this.bb = bb;
  return this;
}

static getRootAsEmbeddedTable(bb:flatbuffers.ByteBuffer, obj?:EmbeddedTable):EmbeddedTable {
  return (obj || new EmbeddedTable()).__init(bb.readInt32(bb.position()) + bb.position(), bb);
}

static getSizePrefixedRootAsEmbeddedTable(bb:flatbuffers.ByteBuffer, obj?:EmbeddedTable):EmbeddedTable {
  bb.setPosition(bb.position() + flatbuffers.SIZE_PREFIX_LENGTH);
  return (obj || new EmbeddedTable()).__init(bb.readInt32(bb.position()) + bb.position(), bb);
}

v(index: number):number|null {
  const offset = this.bb!.__offset(this.bb_pos, 4);
  return offset ? this.bb!.readUint8(this.bb!.__vector(this.bb_pos + offset) + index) : 0;
}

vLength():number {
  const offset = this.bb!.__offset(this.bb_pos, 4);
  return offset ? this.bb!.__vector_len(this.bb_pos + offset) : 0;
}

vArray():Uint8Array|null {
  const offset = this.bb!.__offset(this.bb_pos, 4);
  return offset ? new Uint8Array(this.bb!.bytes().buffer, this.bb!.bytes().byteOffset + this.bb!.__vector(this.bb_pos + offset), this.bb!.__vector_len(this.bb_pos + offset)) : null;
}

static startEmbeddedTable(builder:flatbuffers.Builder) {
  builder.startObject(1);
}

static addV(builder:flatbuffers.Builder, vOffset:flatbuffers.Offset) {
  builder.addFieldOffset(0, vOffset, 0);
}

static createVVector(builder:flatbuffers.Builder, data:number[]|Uint8Array):flatbuffers.Offset {
  builder.startVector(1, data.length, 1);
  for (let i = data.length - 1; i >= 0; i--) {
    builder.addInt8(data[i]!);
  }
  return builder.endVector();
}

static startVVector(builder:flatbuffers.Builder, numElems:number) {
  builder.startVector(1, numElems, 1);
}

static endEmbeddedTable(builder:flatbuffers.Builder):flatbuffers.Offset {
  const offset = builder.endObject();
  builder.requiredField(offset, 4) // v
  return offset;
}

static createEmbeddedTable(builder:flatbuffers.Builder, vOffset:flatbuffers.Offset):flatbuffers.Offset {
  EmbeddedTable.startEmbeddedTable(builder);
  EmbeddedTable.addV(builder, vOffset);
  return EmbeddedTable.endEmbeddedTable(builder);
}

unpack(): EmbeddedTableT {
  return new EmbeddedTableT(
    this.bb!.createScalarList<number>(this.v.bind(this), this.vLength())
  );
}


unpackTo(_o: EmbeddedTableT): void {
  _o.v = this.bb!.createScalarList<number>(this.v.bind(this), this.vLength());
}
}

export class EmbeddedTableT implements flatbuffers.IGeneratedObject {
constructor(
  public v: (number)[] = []
){}


pack(builder:flatbuffers.Builder): flatbuffers.Offset {
  const v = EmbeddedTable.createVVector(builder, this.v);

  return EmbeddedTable.createEmbeddedTable(builder,
    v
  );
}
}