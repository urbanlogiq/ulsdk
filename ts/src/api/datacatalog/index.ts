 ...
 * @param expectedMetadataRevision - Revision the caller expects the metadata object to be at; a stale value is rejected with 409 (verbatim writes only)
 * @param expectedStreamRevision - Revision the caller expects the stream object to be at; a stale value is rejected with 409 (verbatim writes only)
...
export async function updateMetadata(
  ...
  merge: boolean | null,
  comment: string | null,
  expectedMetadataRevision: ContentId | null,
  expectedStreamRevision: ContentId | null,
  ...
  const params: [string, string][] = [];
  if (merge != null) {
      params.push([`merge`, merge.toString()]);
  }
  if (comment != null) {
      params.push([`comment`, comment]);
  }
  if (expectedMetadataRevision != null) {
      params.push([`expected_metadata_revision`, expectedMetadataRevision.toString()]);
  }
  if (expectedStreamRevision != null) {
      params.push([`expected_stream_revision`, expectedStreamRevision.toString()]);
  }
