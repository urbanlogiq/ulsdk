...
impl Serialize for TensorDim<'_> {
    fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
    where
        S: Serializer,
    {
        let mut s = serializer.serialize_struct("TensorDim", 2)?;
...
impl Serialize for Tensor<'_> {
    ...
    where
        S: Serializer,
    {
        let mut s = serializer.serialize_struct("Tensor", 5)?;
        ...
        if let Some(f) = self.strides() {
            s.serialize_field("strides", &f)?;
        } else {
            s.skip_field("strides")?;
        }
