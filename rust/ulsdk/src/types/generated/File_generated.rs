...
impl Serialize for Block {
        ...
        S: Serializer,
    {
        ...
        s.serialize_field("metaDataLength", &self.metaDataLength())?;
...
impl Serialize for Footer<'_> {
    ...
    where
        S: Serializer,
    {
        let mut s = serializer.serialize_struct("Footer", 5)?;
        ...
        if let Some(f) = self.dictionaries() {
            s.serialize_field("dictionaries", &f)?;
        } else {
            s.skip_field("dictionaries")?;
        }
        if let Some(f) = self.recordBatches() {
            s.serialize_field("recordBatches", &f)?;
        } else {
            s.skip_field("recordBatches")?;
        }
